    ...
    <section className="py-12 bg-white border-b border-gray-100">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        {categories.map((category, index) => (
          ...
          >
            <div className="bg-white border-2 border-gray-200 hover:border-gray-300 p-6 sm:p-10 text-center transition-colors duration-300">
              <div className="mb-6 sm:mb-8 flex items-center justify-center">
                <img
                  src={category.image}
                  alt={category.name}
                  className="w-20 h-20 sm:w-28 sm:h-28 object-contain bg-gray-50 border border-gray-200"
                />
              </div>
              <h3 className="font-medium text-xl sm:text-2xl text-gray-900 mb-3 group-hover:text-gray-700 transition-colors duration-300">
                {category.name}
              </h3>
              <p className="text-sm sm:text-base text-gray-500 mb-4">{category.count}</p>
